...
def get_lorenz_data(n_ics, noise_strength=0):
    ...
    data['dx'] = data['dx'].astype(np.float32)
    data['ddx'] = data['ddx'].astype(np.float32)
...
def generate_lorenz_data(ics, t, n_points, linear=True, normalization=None,
    ...
    data['y_spatial'] = y_spatial.astype(np.float32)
    data['modes'] = modes.astype(np.float32)
    data['x'] = x.astype(np.float32)
    data['dx'] = dx.astype(np.float32)
    data['ddx'] = ddx.astype(np.float32)
    data['z'] = z.astype(np.float32)
    data['dz'] = dz.astype(np.float32)
    data['ddz'] = ddz.astype(np.float32)
