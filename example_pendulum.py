...
def get_pendulum_data(n_ics):
    ...
    data['x'] = x.reshape((n_ics*t.size, -1)).astype(np.float32)
    data['dx'] = dx.reshape((n_ics*t.size, -1)).astype(np.float32)
    data['ddx'] = ddx.reshape((n_ics*t.size, -1)).astype(np.float32)
    data['z'] = z.reshape((n_ics*t.size, -1))[:,0:1].astype(np.float32)
    data['dz'] = z.reshape((n_ics*t.size, -1))[:,1:2].astype(np.float32)
